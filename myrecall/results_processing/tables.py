...
from myrecall.results_processing.utils import (
...
def calculate_data_at_the_end(data, index_columns):
    ...
    data_at_the_end = data_at_the_end.groupby(index_columns).tail(10)
...
def calculate_forgetting_individual(data, columns_to_add=["cl_method"]):
    ...
    for env in sorted(data["train/active_env"].unique()):
        ...
        env_data = env_data.groupby(index_columns).tail(10)
...
def calculate_metrics(data, baseline_data, group_by=["cl_method"], methods_order=None):
...
def calculate_intro_metrics(data, group_by=["cl_method"]):
    data_at_the_end, success_rate_of_active_env, individual_forgetting = calculate_forgetting_individual(
        data, columns_to_add=group_by
    )

    active_envs_columns = get_individual_task_success_columns(data)
    t1_performance = compute_mean_and_ci(data_at_the_end, "t1_performance", pd.Index([active_envs_columns[0]]))
    t2_performance = compute_mean_and_ci(data_at_the_end, "t2_performance", pd.Index([active_envs_columns[1]]))
    t1_forgetting = compute_mean_and_ci(individual_forgetting, "t1_forgetting", pd.Index([active_envs_columns[0]]))

    result = pd.concat([t1_performance, t2_performance, t1_forgetting], axis=1)
